...
function App() {
  ...
  useEffect(() => {
    async function getBlockData() {
      ...
      let NFTtokenId = "3478"
      const NFTmetaData = await alchemy.nft.getNftMetadata(NFTcontractAddress, NFTtokenId)
  ...
  return (
    <>
      ...
      <div className="App">Gas Price: {gasPrice ? convertWeiToGwei(gasPrice.toString()) : 'Loading...'} gwei</div>
      <div className='App'>
        Last 10 Blocks: 
        <br></br>
        { "(Click on a block number you want to see the details!)"}
        {blockList ? blockList.map((ele) => (
          <div key={ele} onClick={() => handleBlockClick(ele)}> 
            <span className={selectedBlock && selectedBlock.number === ele ? 'selected-block' : ''}>
              {ele}
            </span>
          </div>)) : "Loading..."}
      </div>
      ...
      {/* <div className="App">
        ...
        <div className="App">
          gasUsed: {txReceipt ? (parseInt(txReceipt.gasUsed._hex)).toString() : '...'}
        </div>
        <br></br>
        <div className='App'>
          BAYC floor price: {NFTfloorPrice ? `${NFTfloorPrice.openSea.floorPrice} ETH` : "Loading..."}
        </div>
        <br></br>
        <div className='App'>
          BAYC 3478 Metadata: 
